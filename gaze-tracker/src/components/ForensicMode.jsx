...
export default function ForensicMode({ faceDetected, onBack }) {
  ...
  // HUD DOM refs — direct manipulation, zero React re-renders in RAF loop
  ...
  const hudLock = (el) => {
    ...
    if (hudHashRef.current) {
      ...
      hudHashRef.current.textContent = h !== '—' ? h.slice(0, 18) + '…' : '—';
  ...
  // ── File reading — hash computed via crypto.subtle ─────────────────────────
  const readFiles = useCallback((fileList) => {
    Array.from(fileList).forEach((file) => {
      const id = ++fileIdRef.current;

      // Read ArrayBuffer for SHA-256, then read again as content
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#050510', overflow: 'hidden' }}>
      ...
      <div id="cyber-hud" style={{
        ...
        <div style={{
          ...
          <div
            ...
            style={{
              ...
              background: '#22d3ee', boxShadow: '0 0 4px #22d3ee',
              flexShrink: 0,
            }}
          />
        ...
        <div style={{ padding: '.55rem .75rem', display: 'flex', flexDirection: 'column', gap: '.42rem' }}>
          ...
          <div style={{ borderTop: '1px solid rgba(0,255,136,0.07)', marginTop: '.1rem', paddingTop: '.1rem' }} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '.2rem', marginTop: '.1rem' }}>
            ...
            <span
              ...
              title=""
              style={{
                color: '#34d399', fontSize: '.65rem', letterSpacing: '.04em',
                background: 'rgba(0,255,136,0.04)', borderRadius: 4,
                padding: '.2rem .4rem', wordBreak: 'break-all',
              }}
            >—</span>
          </div>
        </div>
      ...
      {/* ── Scrollable body ── */}
      <div style={{ position: 'fixed', inset: '48px 0 0', overflowY: 'auto', padding: '1.5rem' }}>
        ...
        <div
          ...
          style={{
            ...
            background: isDragOver ? 'rgba(99,102,241,0.07)' : 'rgba(255,255,255,0.02)',
            padding: '1.75rem', textAlign: 'center',
            transition: 'border-color .2s, background .2s',
          }}
        >
          <div style={{ fontSize: '1.8rem', marginBottom: '.4rem' }}>📁</div>
          <p style={{ color: '#94a3b8', fontSize: '.86rem', marginBottom: '.9rem', lineHeight: 1.6 }}>
            Log ve görüntü dosyalarını buraya sürükleyin<br />
            <span style={{ fontSize: '.74rem', color: '#475569' }}>
              .jpg · .png · .gif · .txt · .log — Dosyalar sunucuya gönderilmez
            </span>
          </p>
          <label style={{
            display: 'inline-block', padding: '.45rem 1.3rem', borderRadius: 9, cursor: 'pointer',
            background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', color: '#fff',
            fontWeight: 700, fontSize: '.82rem', boxShadow: '0 2px 10px rgba(99,102,241,0.38)',
          }}>
            Dosya Seç
            <input
              type="file" multiple
          ...
          {files.length > 0 && (
            <span style={{ marginLeft: '.9rem', fontSize: '.78rem', color: '#818cf8', fontWeight: 600 }}>
              {files.length} delil yüklendi
            </span>
          )}
        </div>
        ...
        {/* Evidence board */}
        {files.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '2.5rem', color: '#475569', fontSize: '.86rem' }}>
            Henüz delil yüklenmedi. Dosya seçerek incelemeye başlayın.
          </div>
        ) : (
          <div style={{ position: 'relative' }}>

            {/* Evidence container — blur applied here */}
            <div
            ...
            >
              {/* Image grid */}
              {imageFiles.length > 0 && (
                <div style={{ marginBottom: textFiles.length ? '1.5rem' : 0 }}>
                  <div style={{
                    fontSize: '.74rem', fontWeight: 700, color: '#818cf8',
                    letterSpacing: '.07em', textTransform: 'uppercase', marginBottom: '1rem',
                  }}>
                    🖼 Görüntü Delilleri — {imageFiles.length} dosya
                  </div>
                  <div style={{
                    display: 'grid',
                    ...
                    {imageFiles.map((f) => (
                      <div
                        ...
                        data-evidence-name={f.name}
                        data-evidence-size={f.size}
                        data-evidence-type={f.fileType}
                        data-evidence-hash={f.hash}
                        style={{
                      ...
                      >
                        ...
                        />
                        <div style={{
                          padding: '.35rem .55rem', fontSize: '.68rem',
              ...
              {textFiles.map((f) => (
                ...
                >
                  <div style={{
                    ...
                    letterSpacing: '.07em', textTransform: 'uppercase', marginBottom: '.7rem',
                  }}>
                    📄 {f.name}
                  </div>
                  <div style={{
                    borderRadius: 9, border: '1px solid rgba(255,255,255,0.05)',
                    background: 'rgba(0,0,0,0.45)', maxHeight: 420, overflowY: 'auto',
                    ...
                    {f.data.split('\n').slice(0, MAX_LINES).map((line, idx) => (
                      ...
                      >
                        <span style={{
                          minWidth: 44, padding: '.18rem .55rem',
                        ...
                        <span style={{
                          padding: '.18rem .7rem', fontSize: '.76rem', color: '#cbd5e1',
                    ...
                    {f.data.split('\n').length > MAX_LINES && (
                      <div style={{
                        padding: '.4rem 1rem', fontSize: '.72rem', color: '#f59e0b',
            ...
            {/* Security overlay — sibling of blurred container, stays sharp */}
            {!faceDetected && (
              <div style={{
                position: 'absolute', inset: 0, borderRadius: 14, zIndex: 50,
